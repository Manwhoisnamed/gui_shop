...
//this is the file for the salesman creator
...
#ifndef __gui_salesman_H
#define __gui_salesman_H 2016
class gui_salesman: public Fl_Window{
    Fl_Input name;
    Fl_Input wage;
    Fl_Input newpass1;
    Fl_Input newpass2;
	...
	((gui_salesman*)data)->cancel_clicked_i();
    }
	...
	//if the wage isn't a double
	if(!isDouble(wage.value())){
	    valid = false;
	    wage_error.label("Needs Double");	
	}
	else{
	    wage_error.label("");
	}
	...
	if(valid){
	    salesman associate(atof(wage.value()), name.value(), atoi(newpass1.value()));
	    database.addAssociate(associate);
	    database.saveData();
	    this->hide();
	}
    }
    static void confirm_clicked(Fl_Widget* w, void* data){
	((gui_salesman*)data)->confirm_clicked_i();
    }
    ...
    gui_salesman():
	Fl_Window(350, 155, "Salesman Creator"),
	name(125, 5, 100, 25, "Name"),
	name_error(230, 5, 100, 25, ""),
	wage(125, 35, 100, 25, "Wage (USD)"),
	wage_error(230, 35, 100, 25, ""),
