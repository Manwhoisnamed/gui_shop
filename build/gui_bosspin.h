//maxwell sanders
//this is the thing that pops up when the boss wants to change his pin
...
#ifndef __gui_bosspin_H
#define __gui_bosspin_H 2016
class gui_bosspin : public Fl_Window{
    Fl_Input oldpass;
    Fl_Input newpass1;
    Fl_Input newpass2;
    Fl_Box old_error;
	...
	((gui_bosspin*)data)->cancel_clicked_i();
    }
	...
	if(atoi(oldpass.value()) != database.getBossPin()){
	    valid = false;
	...
	if(valid){
	    this->hide();
	    database.changeBossPin(atoi(newpass1.value()));
	    database.saveData();
	}
    }
    static void confirm_clicked(Fl_Widget* w, void* data){
	((gui_bosspin*)data)->confirm_clicked_i();
    }
    ...
    void sys_res_vals(){
	oldpass.value(to_string(database.getBossPin()).c_str());
	oldpass.deactivate();
	newpass1.value("");
	newpass2.value("");
	old_error.label("");
    ...
    gui_bosspin():
	Fl_Window(350, 125, "Boss Password Changer"),
