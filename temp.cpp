...
void login_button_clicked(Fl_Widget* w, void* data){
    ...
    Fl_Window* PM_Window = (Fl_Window*) (father->child(1));
    Fl_Box* response = (Fl_Box*) login_window->child(4);
    Fl_Input* input = (Fl_Input*) login_window->child(1);
    Fl_Box* clearance = (Fl_Box*) login_window->child(2);
    if(1337 == atof(input->value()) && strcmp("Project Manager", clearance->label()) == 0){	
  	login_window->hide();
	PM_Window->show();
...
void logout_button_clicked(Fl_Widget* w, void* current){
    ...
    Fl_Window* father = (Fl_Window*)(current_window->parent());
    Fl_Window* login_window = (Fl_Window*) (father->child(0));
    current_window->hide();
    login_window->show();
